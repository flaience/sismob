...
let GenericConfigService = class GenericConfigService {
    ...
    async findAll(tableName, tenantId, search) {
        try {
            const table = schema[tableName];
            const conds = [(0, drizzle_orm_1.eq)(table.tenant_id, tenantId)];
    ...
    async upsert(tableName, dto, tenantId) {
        try {
            ...
            throw new common_1.InternalServerErrorException(`Erro ao salvar em ${tableName}: ${e.message}`);
        }
    }
    // Remove registro

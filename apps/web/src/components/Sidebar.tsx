...
export default function Sidebar() {
  ...
  // --- RECONQUISTA DO PODER (Luis vê tudo) ---
  ...
  return (
    <>
      {/* 🖥️ VERSÃO DESKTOP (Sidebar Lateral) */}
      <aside
          ...
          {menuGroups.map((group) => (
            <div key={group.group} className="space-y-1">
              <button
                ...
                className={`w-full flex items-center justify-between p-4 rounded-2xl transition-all ${openGroup === group.group ? "bg-slate-50 text-brand" : "text-slate-400 hover:bg-slate-50"}`}
              >
                ...
                <div className="ml-6 flex flex-col gap-1 mt-2 animate-in slide-in-from-top-2 duration-300">
                  {group.items.map((item: any) => (
                    <Link
                      key={item.href}
                      href={item.href}
                      className={`flex items-center gap-4 p-3 rounded-xl text-[10px] font-black uppercase tracking-widest ${pathname === item.href ? "text-brand bg-indigo-50" : "text-slate-400 hover:text-brand"}`}
                    >

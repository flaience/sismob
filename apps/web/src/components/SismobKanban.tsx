...
export default function SismobKanban({ tenantId }: any) {
  ...
  useEffect(() => {
    api.get("/negociacoes", { params: { imobiliariaId: tenantId } }).then(res => setCards(res.data));
  }, [tenantId]);

  return (
    <div className="flex gap-6 overflow-x-auto pb-10 custom-scrollbar">
      {ESTAGIOS.map(col => (
        <div key={col.id} className="flex-1 min-w-[320px] bg-slate-100/50 p-6 rounded-[3rem] border border-slate-200/50">
          <div className="flex items-center justify-between mb-6 px-4">
            <h3 className="font-black text-slate-800 uppercase tracking-tighter">{col.label}</h3>
            ...
            {cards.filter(c => c.status === col.id).map(card => (
              ...
              >
                <div className="flex justify-between items-start mb-4">
                   <span className={`text-[9px] font-black uppercase px-3 py-1 rounded-lg text-white ${col.color}`}>
                     {card.intensidade}
                   </span>
                   <p className="text-[10px] font-bold text-slate-300">#{card.id}</p>
                </div>
                <h4 className="font-black text-slate-800 leading-tight mb-1">{card.cliente_nome}</h4>
                <p className="text-xs text-slate-400 font-medium mb-4">{card.imovel_titulo}</p>
                
                <div className="flex justify-between items-center pt-4 border-t border-slate-50">
                  <p className="text-sm font-black text-indigo-600">R$ {Number(card.valor_proposta).toLocaleString()}</p>
